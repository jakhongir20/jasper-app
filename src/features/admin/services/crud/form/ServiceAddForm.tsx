...
export const ServiceAddForm: FC<Props> = ({ open, onCancel, onSuccess }) => {
  const [form] = Form.useForm();
  const { t } = useTranslation();
  const queryClient = useQueryClient();

  const { mutate, isPending: isLoading } = useCreateService({
    onSuccess: () => {
      showGlobalToast(t("common.messages.serviceCreated"), "success");
  ...
  return (
    <Modal
      title={t("common.labels.addService")}
      open={open}
