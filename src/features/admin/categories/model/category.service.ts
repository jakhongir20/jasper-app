...
export class CategoryService {
    static async getAll(params?: GetCategoriesParams): Promise<PaginatedCategoriesResponse> {
    ...
    static async getById(categoryId: number): Promise<Category> {
        return await apiService.$get<Category>(`/admin/category?category_id=${categoryId}`);
    }
    ...
    static async update(payload: UpdateCategoryPayload): Promise<Category> {
        ...
        return await apiService.$put<Category>(`/admin/category?category_id=${category_id}`, updateData);
    }

    static async delete(categoryId: number): Promise<void> {
        return await apiService.$delete(`/admin/category?category_id=${categoryId}`);
    }
}
