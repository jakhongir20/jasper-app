...
import { baseboardFormFields } from "@/features/dashboard/bids";
import {
...
interface Props {
  ...
  closeModal: () => void;
  title: string;
  initialValues?: ApplicationBaseboard;
}

export const BaseboardModal: FC<Props> = ({
  isOpen,
  closeModal,
  title,
  initialValues,
  ...
  useEffect(() => {
    if (isOpen) {
      if (initialValues) {
        // If editing, use provided baseboard values directly
  ...
  const handleConfirm = () => {
    form.validateFields().then((values) => {
      ...
      const currentAspects = parentForm.getFieldValue("baseboards") || [];

      if (initialValues) {
        // Update existing baseboard
        const updatedAspects = currentAspects.map(
          (baseboard: ApplicationBaseboard) =>
            baseboard._uid === initialValues._uid
              ? { ...formValues }
              : baseboard,
        );

        parentForm.setFieldsValue({
          baseboards: updatedAspects,
        });
      } else {
        const baseboards = [
          ...currentAspects,
  ...
  return (
    ...
    >
      <Form layout="vertical" form={form} className="grid gap-4 p-5">
        ...
        {baseboardFormFields.map(renderFormItem)}
      </Form>
    </Modal>
  );
};
