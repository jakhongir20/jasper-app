...
import { columns } from "@/features/dashboard/bids/constants/columns/TableServiceColumn";
import { useToast } from "@/shared/hooks";
import { ServiceModal } from "@/features/dashboard/bids/crud/modal/ServiceModal";
...
export const TabServicesForm: FC<Props> = ({
  className,
  mode,
  ...
  const [editingItem, setEditingItem] = useState<ApplicationService | null>(
    null,
  );
  const [itemToDelete, setItemToDelete] = useState<ApplicationService | null>(
    null,
  );

  const onOpenDelete = (record: ApplicationService) => {
  ...
  const onEditItem = (record: ApplicationService) => {
  ...
  return (
    <div className={cn("relative py-1", className)}>
      <TableWrapper<ApplicationService>
